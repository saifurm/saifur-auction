...
const PREVIEW_SCENES: Array<{
...
}> = [
  {
    key: "lobby",
    label: "Lobby",
    title: "Curate the velvet-rope lobby",
    description: "Share the secret password, see who's joined, and keep the vibe exclusive.",
    variant: "lobby"
  },
  {
    key: "auction",
    label: "Live Auction",
    title: "Call shots in real time",
    description: "On-deck intel, timer pressure, and bidding controls in one deck.",
    variant: "auction"
  },
  {
    key: "playerlist",
    label: "Player List",
    title: "Command the entire board",
    description: "Current, upcoming, and earlier picks stay crystal clear for every host.",
    variant: "playerlist"
  },
  {
    key: "unsold",
    label: "Unsold Board",
    title: "Bring stars back instantly",
    description: "Tap re-open to relist icons without touching spreadsheets.",
    variant: "unsold"
  },
  {
    key: "lock",
    label: "Lock Final XI",
    title: "Drop players on a real pitch",
    description: "Soccer or cricket views render like a broadcaster's control room.",
    variant: "lock"
  },
  {
    key: "rank",
    label: "Rank Others",
    title: "Stack rivals with style",
    description: "Drag cards, inspect their lineup, and reward the sharpest roster.",
    variant: "rank"
  },
  {
    key: "leaderboard",
    label: "Leaderboard",
    title: "Publish cinematic results",
    description: "Formations, payouts, and prestige on one champagne-ready board.",
    variant: "leaderboard"
  }
];
...
const PreviewShowcase = () => (
  <div className="landing-preview">
    <div>
      ...
      <h3>Tour the entire flow before you play</h3>
    </div>
    <div className="preview-grid">
      {PREVIEW_SCENES.map((scene) => (
        <article key={scene.key} className="preview-card">
          <span className="preview-chip">{scene.label}</span>
          <h4>{scene.title}</h4>
          <p>{scene.description}</p>
          <PreviewMock variant={scene.variant} />
        </article>
      ))}
    </div>
  </div>
...
const PreviewMock = ({ variant }: { variant: PreviewVariant }) => {
  switch (variant) {
    case "lobby":
      return (
        <div className="mock-screen">
          <div className="mock-row space">
            <span className="mock-pill glow">Soccer Auction</span>
            <span className="mock-pill">Joined 2 / 3</span>
          </div>
          <div className="mock-row">
            <span className="mock-pill">Password • 12345</span>
            <span className="mock-pill muted">Tap copy</span>
          </div>
          <div className="mock-list">
            <span className="mock-line strong">Saifur • Admin</span>
            <span className="mock-line">Navid</span>
            <span className="mock-line">Shanto</span>
          </div>
        </div>
      );
    case "auction":
      return (
        <div className="mock-screen tall">
          <div className="mock-row space">
            <span className="mock-pill glow">On deck</span>
            <span className="mock-timer">00:12</span>
          </div>
          <h5>Lionel Messi</h5>
          <p>Category A • Base $70</p>
          <div className="mock-bid-grid">
            <div className="mock-value">$91</div>
            <div className="mock-button">Bid</div>
          </div>
          <div className="mock-mini-grid">
            <span>My team · 3 signed</span>
            <span>Other teams · 2 signed</span>
          </div>
        </div>
      );
    case "playerlist":
      return (
        <div className="mock-screen">
          <div className="mock-row">
            <span className="mock-pill glow">Now drafting</span>
            <span className="mock-pill muted">Paused</span>
          </div>
          <ol className="mock-order">
            <li>#7 Rodri</li>
            <li>#8 Harry Kane</li>
            <li>#9 Lionel Messi</li>
            <li>#10 Lamine Yamal</li>
          </ol>
        </div>
      );
    case "unsold":
      return (
        <div className="mock-screen">
          <div className="mock-row space">
            <span className="mock-pill glow">Unsold</span>
            <span className="mock-pill">Ready</span>
          </div>
          <div className="mock-unsold">
    ...
    case "lock":
      return (
        <div className="mock-screen">
          <div className="mock-row space">
            <span className="mock-pill glow">Lock XI</span>
            <span className="mock-pill muted">4-3-3</span>
          </div>
          <div className="mock-field">
            {PREVIEW_FIELD_POSITIONS.map((slot) => (
              <span
                key={slot.label}
                className="mock-player"
                style={{ left: `${slot.x}%`, top: `${slot.y}%` }}
              >
                {slot.label.replace(/\d/, "")}
              </span>
            ))}
          </div>
        </div>
    ...
    case "rank":
      return (
        <div className="mock-screen">
          ...
          <div className="mock-field micro">
            {PREVIEW_FIELD_POSITIONS.slice(0, 6).map((slot) => (
              <span
                key={`rank-${slot.label}`}
                className="mock-player"
                style={{ left: `${slot.x}%`, top: `${slot.y}%` }}
    ...
    case "leaderboard":
      return (
        <div className="mock-screen">
          <div className="mock-row space">
            <span className="mock-pill glow">Final leaderboard</span>
            <span className="mock-pill muted">New</span>
          </div>
